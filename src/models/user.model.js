...
async function generateUniqueRegistrationNumber() {
  ...
  do {
    ...
    // Use already-registered User model
...
const userSchema = new mongoose.Schema(
  {
    ...
    password: {
      ...
      default: "",
      required: function () {
        // EMPLOYEE → mobile + PIN only → password NOT required
        if (this.role === "EMPLOYEE") {
          return false;
        }
        // OWNER / USER / ADMIN → password required for local provider
    ...
    // Auto-generated TNMA-YYYY-XXXXXX
    RegistrationNumber: {
      type: String,
      unique: true,
    },
    ...
    // 🔑 roles (for your whole ecosystem)
    role: {
      type: String,
    ...
    // For linking employees to owners, etc.
    parentOwner: {
      type: mongoose.Schema.Types.ObjectId,
      ref: "User",
      default: null,
...
userSchema.pre("save", async function (next) {
  try {
    ...
    // If already set externally, keep it
